  ...
  <div class="overflow-x-auto">
    <table class="w-full text-sm">
      ...
      <tbody>
        {#each data as row}
          <tr
            class="border-b border-gray-100 hover:bg-primary-50 transition-colors {onRowClick ? 'cursor-pointer' : ''}"
                ...
                {#if col.key === 'persentase'}
                  <div class="flex items-center gap-2">
                    <div class="w-16 bg-gray-200 rounded-full h-2">
                      <div
                        class="h-2 rounded-full transition-all duration-300 {row[col.key] >= 80 ? 'bg-green-500' : row[col.key] >= 50 ? 'bg-yellow-500' : 'bg-red-500'}"
